...
jsObject.Setted = {
    ...
    init: function () {
        ...
        //调整合同额更改
        $('#htAmount').change(function () {

            var _htAmount = this.value;
            var _syAmount = $('#syAmount').val();

            $('#tzsyAmount').val(_htAmount - _syAmount);
        });
    ...
    InitTable: function () {
        ...
        $('#table1').bootstrapTable({
            ...
            url: '../Hrm/GetHr_Crm_tzys',        //后台数据url
    ...
    Export: function () {
        ...
        if (dataJson.length > 0) {
            ...
            window.open("../Hrm/ExportHr_Crm_tzys?startDate=" + _startDate + "&endDate=" + _endDate);
        }
        else {
            $('#result').html("当前无数据导出！");
            $("#myModal").modal("show");
    ...
    SaveData: function () {
        ...
        $.ajax({
            type: 'POST',
            url: '../Hrm/SaveHr_Crm_tzys',
            data: { id: $('#hrId').val(), htAmount: $('#htAmount').val(), syAmount: $('#syAmount').val() },
            success: function (data) {
...
operateEvents = {
    ...
    'click .RoleOfB': function (e, value, row, index) {
        ...
        $('#syAmount').val(row.syAmount);
        $('#ghsyAmount').val(row.ghsyAmount);
        $('#tzsyAmount').val(row.tzsyAmount);

        $('#crmComName').html(row.crmComName);
        $('#crmDeptName').html(row.crmDeptName);
        ...
        //jsObject.Setted.DelData(value); //修改
    }
};
