...
jsObject.Setted = {
        ...
        $('#table1').bootstrapTable({
            ...
            columns: [
                ...
                },
                {
                    ...
                    formatter: jsObject.Setted.operateFormatter 
                }
            ],

            url: '../Hrm/GetHr_Midtzys',        //后台数据url
    ...
    operateFormatter: function (value, row, index) {
        return [
               '<a class="RoleOfB btn btn-info btn-xs" href="#"><i class="fa fa-edit"></i>&nbsp;修改</a>'
        ].join('');
    },
    ...
    Export: function () {
        ...
        if (dataJson.length > 0) {
            ...
            window.open("../Hrm/ExportHr_Midtzys?startDate=" + _startDate + "&endDate=" + _endDate);
        }
        else {
            $('#result').html("当前无数据导出！");
            $("#myModal").modal("show");
    ...
    SynData: function () {
        ...
        $.ajax({
            type: 'POST',
            url: '../Hrm/SynHr_Midtzys',
            data: { yearly: $('#yearly2').val(), monthly: $('#monthly2').val() },
            success: function (data) {

                $('#msg1').html(data);

                $("#btnSyn").attr('disabled', false);
    ...
    SaveData: function () {
        ...
        $.ajax({
            type: 'POST',
            url: '../Hrm/SaveHr_Midtzys',
            data: {
                id: $('#hrId').val(), cxNum: $('#cxNum').val(), htAmount: $('#htAmount').val(), ysAmount: $('#ysAmount').val(),
                lrAmount: $('#lrAmount').val(), yieldVal: $('#yield').val(), yieEffic: $('#yieEffic').val(),
                gjEffic: $('#gjEffic').val(),proTeams: $('#proTeams').val(), workDays: $('#workDays').val()
            },
            success: function (data) {
...
operateEvents = {
    ...
    'click .RoleOfB': function (e, value, row, index) {
        ...
        $('#hrId').val(row.id);
        $('#cxNum').val(row.cxNum);
        $('#htAmount').val(row.htAmount);
        $('#ysAmount').val(row.ysAmount);
        $('#lrAmount').val(row.lrAmount);
        $('#yield').val(row.yield);
        $('#yieEffic').val(row.yieEffic);
        $('#gjEffic').val(row.gjEffic);
        $('#proTeams').val(row.proTeams);
        $('#workDays').val(row.workDays);

        $('#company').html(row.comName);
        $('#yearly').html(row.yearly);
        $('#monthly').html(row.monthly);
